...
do
  ...
  # avoid clobbering the input (e.g. already .mp3) or an existing file
  if [ "$newfilename" = "$file" ] || [ -e "$newfilename" ]; then
    c=1
    while [ -e "${file%.*} ($c).mp3" ] || [ "${file%.*} ($c).mp3" = "$file" ]; do
      ((c+=1))
    done
    newfilename="${file%.*} ($c).mp3"
  fi
  ...
  docker run --rm --user 1000:1000 -v "$(pwd)":/work -w /work jrottenberg/ffmpeg:4.1-scratch \
    -nostats -i "$file" -vn -c:a libmp3lame -q:a 2 -progress pipe:1 -y "$newfilename" 2>/dev/null |
...
zenity --progress --title="Convert Audio to MP3" --text="starting..." --percentage=1
